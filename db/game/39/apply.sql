ALTER TABLE `account_info` 
	ADD COLUMN `daily_login_streak` INT(4) NULL DEFAULT 0  AFTER `battle_count` ,
	ADD COLUMN `daily_login_time` BIGINT(8) NULL DEFAULT 0  AFTER `daily_login_streak` ,
	ADD COLUMN `daily_login_bonus` INT(4) NULL DEFAULT 0  AFTER `daily_login_time`,
	ADD COLUMN `last_online` BIGINT(8) NULL DEFAULT 0  AFTER `daily_login_bonus`;
	

update account_info
join
	(select user_id, GREATEST(session_start, session_end) last_time from session_history group by user_id order by session_start limit 10000) t2
on t2.user_id = account_info.account_id
set account_info.last_online=t2.last_time;
...
from
 ...
 left JOIN
	(
		select 
			COUNT(distinct session_history.user_id) as distinct_users,
			@_day _day
